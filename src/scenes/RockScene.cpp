...
Rock sceneRocks[MAX_ROCKS];

 void initRocks() {
     sf::Vector2u textureSize = sceneRocks[0].texture.getSize();

     for (int i = 0; i < MAX_ROCKS; ++i) {
         float x = static_cast<float>(rand() % (SCREEN_W - textureSize.x));
         float y = static_cast<float>(rand() % (SCREEN_H - textureSize.y));
         sceneRocks[i].sprite.setTexture(sceneRocks[i].texture);
         sceneRocks[i].sprite.setPosition(x, y);
     }
 }

 void updateRocks(float dt, int groundheight, int charheight) {
     for (int i = 0; i < MAX_ROCKS; ++i) {
...
void onRockSceneRender(sf::RenderWindow& window) {
    initRocks();
//    updateRocks();
 }
